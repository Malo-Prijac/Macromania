    ...
    <div class="positiontitre">
      ...
      <?php
        if (isset($_SESSION['admin'])){
            ...
            echo "<br>";
            echo "<button id='gestion' onclick=\"window.location.href='php/gestionAccueil.php'\">Gérer accueil</button>";
        } else {
      ...
      if(isset($_POST['ajouterPanier'])||(isset($_POST['acheter']))){
        ...
        if($_POST["quantite"]!=0){
          ...
          if(isset($_POST['acheter'])){
            echo "<script>window.location.replace('php/panier.php')</script>";
          }
        }
      }
    ?>
      ...
      <?php
        ...
        if($connexionSQL){
          ...
          while ($articleAccueil=mysqli_fetch_row($listeAccueil)){
            $result=rechercheArticle($connexionSQL,$articleAccueil[0]);
            while ($article=mysqli_fetch_row($result)){
              ?>
              <div class='column'>
                <img src="<?php echo substr($article[3], 3);  ?>"/>
                <div class='PosTitle'>
